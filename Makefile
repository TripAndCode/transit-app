...
doctor:
	...
	@echo "── git hooks ──"
	@hook="$$(git rev-parse --git-common-dir 2>/dev/null)/hooks/pre-commit"; \
		if [ -x "$$hook" ] && grep -q "File generated by pre-commit" "$$hook" 2>/dev/null; then \
			echo "  gitleaks pre-commit hook installed"; \
		else \
			echo "  gitleaks pre-commit hook MISSING — \`make hooks\` (commits are NOT scanned locally)"; \
		fi
