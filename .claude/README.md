...
- To trigger early, SSH to the VPS and run `/root/claude-loop.sh`; otherwise wait for
  ...
  operates on one item, and the loop may merge its own PR once both review passes
  are clean and it's mergeable/clean — see the guarded-continuation bullet above
