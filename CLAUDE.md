...
Frontend (`cd frontend`): `npm run typecheck && npm run test && npm run lint && npm run lint:i18n && npm run lint:i18n-strings && npm run test:check-entry-chunk && npm run build:bundle && npm run check:entry-chunk` — all seven checks must pass before a PR (the last two enforce "keep MapLibre out of the entry chunk", see `frontend/scripts/check-entry-chunk.mjs`; `build:bundle` skips `tsc -b` since `typecheck` already ran it). `npm run dev` for the hot-reload loop (proxies `/api` to :8000).
...
- **Every PR gets a `/review-branch` pass before it's opened — no exceptions for small/trivial diffs, and this applies identically whether the work originated from an interactive session or the autonomous VPS loop.** The trivial-tier fast path exists for genuinely doc-only diffs **outside `.claude/**` and this file** — a diff touching `.claude/**` (commands, agents, skills, hooks, settings) or `CLAUDE.md` is *process-doc* tier in `review-branch.md`'s risk-tier list, never trivial, because later sessions execute those files rather than read them. It is not a reason to skip the flow entirely.
