...
- React 19.2 with the **React Compiler enabled** — `useMemo`/`useCallback`/`React.memo` are banned as a hard ESLint error (`no-restricted-syntax` + `no-restricted-imports` + `no-restricted-properties` in `frontend/eslint.config.js`; `npm run lint` fails on any use, including the `React.useMemo`/`React.useCallback`/`React.memo` member-expression form, aliased imports, and any other receiver's `.useMemo`/`.useCallback`/`.memo` property access) — the compiler memoizes automatically, so don't add them for performance. For fresh-props-in-stable-handlers, use `useEffectEvent` (see `MapTab`), never render-time ref writes.
