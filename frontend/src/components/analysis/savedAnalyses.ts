...
export function saveAnalysis(agencyId: number, title: string, ctx: RangeCtx, compare: boolean) {
  ...
  localStorage.setItem(KEY, JSON.stringify([{ id: crypto.randomUUID(), agencyId, title, query, savedAt: new Date().toISOString() }, ...rows].slice(0, 100)));
}
export function deleteAnalysis(id: string) {
  localStorage.setItem(KEY, JSON.stringify(readAnalyses().filter((r) => r.id !== id)));
}
