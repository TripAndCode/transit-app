...
export function QueueResizer({ width, label, onWidth }: {
  width: number; label: string; onWidth: (px: number) => void;
}) {
  ...
  return (
    ...
    <div
      ...
      onPointerMove={(e) => {
        if (drag.current) onWidth(clampQueueWidth(drag.current.width - (e.clientX - drag.current.x)));
      }}
      onPointerUp={(e) => {
        drag.current = null;
        e.currentTarget.releasePointerCapture?.(e.pointerId);
      }}
      onDoubleClick={() => onWidth(DEFAULT_QUEUE_WIDTH)}
      onKeyDown={(e) => {
        const step = e.shiftKey ? 48 : 16;
        if (e.key === "ArrowLeft") { e.preventDefault(); onWidth(clampQueueWidth(width + step)); }
        if (e.key === "ArrowRight") { e.preventDefault(); onWidth(clampQueueWidth(width - step)); }
      }}
    />
  );
}
