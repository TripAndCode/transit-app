...
![事業者選択画面](./01-agency-select.ja.png)
...
![Analysisタブ（初期状態）](./04-analysis-landing.ja.png)
...
![ヒントポップアップ](./06-analysis-hint-popover.ja.png)
...
![Delay ranking画面](./05-analysis-ranking.ja.png)
...
![Trendレポート画面](./07-analysis-trend.ja.png)
...
![On-time rateレポート画面](./08-analysis-ontime.ja.png)
...
![注目ポイントパネル（初期表示）](./04-analysis-landing.ja.png)
...
![Viewボタンで絞り込んだ結果](./09-analysis-insight-view.ja.png)
...
![Agencies（比較）タブ](./10-network.ja.png)
...
![Latest observationsタブ](./11-live.ja.png)
...
![Askタブ初期画面](./12-ask-landing.ja.png)
...
- **その下には自由入力欄もあります。** 用意されたボタンにない質問（例：「先週と比べて悪化した路線はある？」）を
  そのまま文章で打ち込み、入力欄の右にある送信ボタン（紙飛行機のアイコン）を押すと、
  今表示されている結果に基づいた回答が返ってきます。

![フォローアップの自由入力欄](./14-ask-followup-freetext.ja.png)
