...
def test_insert_updates_pads_legacy_8_tuple_rows_with_null_scheduled_sec(ch_client):
    """A strategy (or test fixture) built before scheduled_sec (and every
    later trailing column) existed still passes an 8-tuple -- insert_updates
    must pad it with NULL rather than raising a column-count mismatch
    against the now-16-entry UPDATE_COLUMNS."""
    rows = [("a/000001.pb", "2026-01-01T10:00:00Z", "T1", "weekday", "10:00", "R1", 1, 30)]
    n = insert_updates(ch_client, agency_id=7, rows=rows)
    assert n == 1
    result = ch_client.query("SELECT scheduled_sec, static_version_id FROM updates")
    assert result.result_rows == [(None, None)]


def test_insert_updates_stores_scheduled_sec_when_provided(ch_client):
    """static_join.py's shape carries its own scheduled_sec (2nd-to-last of
    UPDATE_COLUMNS), e.g. for a post-midnight "25:30:00" departure_time that
    has no representation in scheduled_time. A row ending right after
    scheduled_sec (omitting the trailing static_version_id) still gets that
    one column padded with NULL."""
    rows = [
        ("a/000001.pb", "2026-01-01T10:00:00Z", "T1", "weekday", None, "R1", 1, 30, None, None, None, None, None, 91800)
    ]
    n = insert_updates(ch_client, agency_id=7, rows=rows)
    assert n == 1
    result = ch_client.query("SELECT scheduled_time, scheduled_sec, static_version_id FROM updates")
    assert result.result_rows == [(None, 91800, None)]
