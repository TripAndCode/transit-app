...
def test_route_baseline_sql_p90_pooling_ignores_null_p90_rows(pg_conn, agency_id):
    ...
    contributing service_type whose own p90_min is null (every contributing
    row's dep_delay was itself NULL -- agg_route_stats no longer gates thin
    groups out at insert time). SUM(p90_min * samples) silently skips a
