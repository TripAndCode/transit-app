...
_LOCALES: dict[tuple[str, str], str] = {
    ...
    ("suggest_reason_anomaly", "en"): "Route {route}'s average delay today is much worse than usual (avg {avg_min} min).",
    ("suggest_reason_trend_shift", "ja"): "路線{route}の遅延が今週の途中から悪化しています（{delta_min}分の変化）。",
    ("suggest_reason_trend_shift", "en"): "Route {route}'s delay pattern shifted partway through this week ({delta_min} min change).",
