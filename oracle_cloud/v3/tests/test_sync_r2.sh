#!/usr/bin/env bash
# sync-r2.sh: syncs each agency's rt/static dir to the right R2 path, and
# fails closed when required OBJECT_STORE_* env vars are missing.
set -euo pipefail
cd "$(dirname "$0")"
source ./helpers.sh
setup_base
...
# per-agency R2 paths, scoped to the right file patterns.
...
pass "each agency's rt/static synced to its own R2 path with the right filters"
