...
# re-derives, file by file and the same way verify-r2.sh does (an actual R2
# listing, byte-size compared against the local copy), whether *this specific*
# archive is confirmed uploaded, and reclaims its disk the moment that is
# true -- independent of prune.sh's much longer retention window. A failed or
# partially uploaded archive (missing from R2, or present with a mismatched
# size) is left in place for the next sync-r2.sh run to retry: this script
# only ever deletes a file it has itself just confirmed is byte-identical in
# R2.
#
...
# r2_size <prefix> <basename> — echoes the byte size of the R2 object under
# s3://$OBJECT_STORE_BUCKET/<prefix> whose basename matches, and returns 0.
# Returns 1 (nothing echoed) when no such object is listed.
r2_size() {
    local prefix="$1" name="$2" d t size key
...
# reclaim_or_keep <local-file> <r2-prefix>
# Removes <local-file> when R2 already holds a byte-identical object with the
# same basename under <r2-prefix>; otherwise leaves it in place (recoverable)
# and adds its size to the running remaining_bytes total.
reclaim_or_keep() {
    local f="$1" prefix="$2" name local_size remote_size
    name=$(basename "$f")
    local_size=$(wc -c < "$f" | tr -d ' ')
    if remote_size=$(r2_size "$prefix" "$name"); then
...
while IFS= read -r row || [ -n "${row:-}" ]; do
    ...
    if [ -d "$rt" ]; then
        while IFS= read -r f; do
            [ -n "$f" ] || continue
            reclaim_or_keep "$f" "rt/$id/"
        done < <(find "$rt" -maxdepth 1 -name '*.tar.gz' -type f)
    fi
    ...
    if [ -d "$sdir" ]; then
        ...
        while IFS= read -r f; do
            ...
            reclaim_or_keep "$f" "static/$id/"
        done < <(find "$sdir" -maxdepth 1 -name 'gtfs_static_*.zip' -type f)
    fi
done < "$TSV"
