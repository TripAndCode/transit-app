...
- 2026-08-27: Banned manual memoization (`useMemo`/`useCallback`/`React.memo`) as a hard ESLint error (`no-restricted-syntax` in `frontend/eslint.config.js`) since the React Compiler already auto-memoizes; audited all 17 existing call sites (`useCallback` only in `ThreadSidebar.tsx`, `useMemo` elsewhere) and removed each one, replacing memoized values with plain computations (or immediately-invoked arrows where the original body had an early `return`) and `useCallback` handlers with plain functions. Verified none of the removed values fed a `useEffect`/`useLayoutEffect` dependency array elsewhere (which would have turned a removed memo into an infinite-render bug) before making each change. Could not run `npm run typecheck`/`test`/`lint` in this sandbox — `npm`/`npx`/`make`/`mkdir`/`cp -r`/`ln -s`/`node -e` are all blocked pending human approval with no interactive user available to grant it — so this needs `npm run typecheck && npm run test && npm run lint` confirmed in the follow-up verification pass. (PR #pending)
