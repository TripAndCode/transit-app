...
python "$SCRIPT_DIR/../gtfs_pipeline.py" ingest "$SCRIPT_DIR/../raw_archives" --agency-id "$AGENCY_ID"

# 3. Load latest static GTFS (idempotent — re-loads only if new zip appears)
echo "==> Loading static GTFS for agency $AGENCY_ID"
python "$SCRIPT_DIR/../gtfs_pipeline.py" load_static "$SCRIPT_DIR/../raw_archives_static" --agency-id "$AGENCY_ID"

# 4. Re-run aggregations
echo "==> Running analysis for agency $AGENCY_ID"
python "$SCRIPT_DIR/../gtfs_pipeline.py" analyze --agency-id "$AGENCY_ID"
